...
to the $\omega(n)\geq2$ part.

\begin{proof}[Proof of Theorem~\ref{thm:identity}]
For fixed $x$, all sums below are finite. Indeed,
$\pi(x^{1/i})=0$ for all sufficiently large $i$, and
$\binom{\omega(n)}{j}=0$ whenever $j>\omega(n)$.
...
\begin{itemize}
    ...
    \item[(iii)] integers with $\omega(n) \geq 2$.
\end{itemize}
...
\]
We claim that
\begin{equation}\label{eq:composite-part-identity}
    \sum_{\substack{n \leq x \\ \omega(n) \geq 2}} d(n)
    = \sum_{j \geq 2} (-1)^j (j-1) T_j(x).
\end{equation}

To prove~\eqref{eq:composite-part-identity}, interchange the order
of summation on the right-hand side. This involves only finite sums,
so no convergence issue occurs:
\[
    \sum_{j \geq 2} (-1)^j (j-1) T_j(x)
...
\]
which establishes~\eqref{eq:composite-part-identity}.
