...
\]
be the Meissel--Mertens constant~\cite{Tenenbaum2015}, and let
\[
    P(2)=\sum_p\frac1{p^2}
\]
...
\]
For every prime $p$, we have $H_p(1,0)=1$, and hence
$H(1,0)=1$. Logarithmic differentiation of the Euler product gives
\[
    \partial_y H(1,0)=\sum_p \partial_y\log H_p(1,0).
\]
Indeed, the summand computed below is $O(p^{-2})$, so the series
converges absolutely and the logarithmic differentiation is justified.
...
$1/\Gamma(\alpha)$~\cite[p.139]{NIST:DLMF},
\[
    \frac1{2\pi i}
