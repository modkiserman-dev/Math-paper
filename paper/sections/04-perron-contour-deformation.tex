...
by~\eqref{eq:L-def}; see, e.g., \textup{[]}.
...
\begin{equation}\label{eq:I-H}
    ...
    F(s, y) \frac{x^s}{s}\, ds.
\end{equation}
Writing $\mathcal{H}_{\mathrm{loc}}
=\mathcal{H}_{\mathrm{loc}}^-\cup\mathcal{H}_{\mathrm{loc}}^\circ
\cup\mathcal{H}_{\mathrm{loc}}^+$ for the lower arm, the small
circle, and the upper arm, respectively, fixes the geometric
decomposition of the local contour.
...
$K\geq2M$ and $K\geq2$. We first show that
\[
...
\]
is increasing in $a$ for $K\geq2$, and at $a=1$ it equals $K/2$.
...
exponentially smaller than the local contribution on the
$A(x,y)$ level. The local Hankel contribution $I_H$, analyzed in
