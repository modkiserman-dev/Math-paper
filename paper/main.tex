...
Voronoï \cite{} first improved Dirichlet's bound to $\theta \leq 1/3$ 
...
\[
    ...
    \quad \text{and} \quad 
    \sum_{j=0}^{m} (-1)^j \, j \binom{m}{j} =\left.\frac{d}{dx}(1-x)^m\right|_{x=1}=0 \ \text{(for } m \geq 1),
\]
...
\begin{equation}\label{eq:zero-free-region}
    ...
    \sigma \geq 1 - \frac{c_0}{\log(|t| + 2)} \right\} 
    \setminus \{1\}.
\end{equation}
\end{theorem}
...
Indeed, $\mathcal{Z} \setminus \{1\}$ is a slit-once-punctured 
domain that is not simply connected, since a small loop around 
$s = 1$ inside $\mathcal{Z}$ is not nullhomotopic. Removing the 
...
every $\sigma > 1/2$ and every $|t| \geq 2$,
\begin{equation}\label{eq:arg-zeta-bound}
    |\arg \zeta(\sigma + it)| \leq C_{\arg} \log|t|,
\end{equation}
where the branch of $\arg \zeta$ is the one determined by 
analytic continuation from $\sigma > 1$ along any path in 
$\mathcal{D}$.
\end{theorem}
 
We now combine these bounds. On the range $|t| \geq 2$ and 
$1 - \delta' \leq \sigma \leq 1 + \delta'$, applying 
Theorem~\ref{thm:zeta-uniform} with $\delta = \delta'$ and 
Theorem~\ref{thm:arg-zeta} to~\eqref{eq:F-safe-bound} gives
\begin{equation}\label{eq:F-bound-large-t}
    |F(\sigma + it, y)| 
    \ll_{\eta, \varepsilon} 
    |t|^{(2 + 2\eta)(\delta'/2 + \varepsilon)} 
    \cdot |t|^{2 \eta C_{\arg}} 
    = |t|^{B},
\end{equation}
where
\begin{equation}\label{eq:B-exponent}
    B := B(\eta, \varepsilon) 
       := (2 + 2\eta)\!\left(\tfrac{\delta'}{2} + \varepsilon\right) 
        + 2 \eta C_{\arg} 
       = (2 + 2\eta)\!\left(\tfrac{1}{8} + \varepsilon\right) 
        + 2 \eta C_{\arg}.
\end{equation}
The implied constant in~\eqref{eq:F-bound-large-t} depends on 
$\eta$ and $\varepsilon$ but not on $t$ or $y$. For $\eta$ and 
$\varepsilon$ sufficiently small, $B < 1$, which is essential for 
the contour estimates of Section~\ref{sec:contour-estimates} to 
vanish in the limit $T \to \infty$.
 
On the complementary range $|t| \leq 2$ and 
$1 - \delta' \leq \sigma \leq 1 + \delta'$ (a compact subset of 
$\mathcal{D}$ avoiding $s = 1$), all of $|H(s, y)|$, $|\zeta(s)|$, 
and $|\arg \zeta(s)|$ are bounded by absolute constants, so 
$|F(s, y)| \ll_\eta 1$ uniformly. Combining the two ranges, we 
obtain on the entire vertical strip 
$1 - \delta' \leq \sigma \leq 1 + \delta'$ (with $s \neq 1$):
\begin{equation}\label{eq:F-bound-uniform}
    ...
    \qquad (|y| \leq \eta).
\end{equation}
This is the bound that will be used throughout 
Section~\ref{sec:contour-estimates}.
...
Section~\ref{sec:hankel}). Write 
$i 0^\pm := \lim_{\varepsilon \to 0^+}(\pm i \varepsilon)$ for 
points on the upper and lower edges of the slit $\mathcal{S}$, 
viewed as distinct points on the Riemann surface of 
$\log(s - 1)$.
...
The Hankel arms (3a) and (3c) follow the slit \(\mathcal S\) on
opposite edges. In the local representation
\[
...
Combined with the fact that $\mathcal{C} \setminus \{1\}$ avoids 
the slit $\mathcal{S}$ (the Hankel arms (3a) and (3c) lie on its 
upper and lower edges, viewed as distinct points on the Riemann 
surface, hence not on $\mathcal{S}$ itself), we conclude that 
$\mathcal{C} \setminus \{1\} \subset \mathcal{D}$. The integrand 
$F(s, y) x^s / s$ is therefore well-defined and holomorphic on a 
neighborhood of $\mathcal{C} \setminus \{1\}$.
...
$\varepsilon$ are chosen sufficiently small that $B < 1$.
...
\[
    ...
    \ll_{\eta, \varepsilon} 
    x^{1 - \lambda_T} 
    \int_{-T}^{T} \frac{(|t| + 2)^B}{\max(1, |t|)}\, dt.
\]
...
\[
    ...
    \ll T^B,
\]
using $B > 0$. This gives~\eqref{eq:vertical-bound}.
\end{proof}
...
$B = (2 + 2\eta)(1/8 + \varepsilon) + 2\eta C_{\arg}$ tends to 
$1/4$ as $\eta, \varepsilon \to 0$, and $c_0 / 2$ is at least 
some absolute constant (in particular, for the standard de la 
Vall\'ee Poussin region one may take $c_0 \geq 1/2$, giving 
$c_0/2 \geq 1/4$; refining $\varepsilon$ adjusts as needed).

Setting $c_1 := \min(1 - o(1),\ 1 - B,\ c_0/2 - B)/2$ for $x$ 
large enough yields~\eqref{eq:total-nonlocal-error}.
\end{proof}
